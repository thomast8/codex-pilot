...
@server.tool(
    description=(
        ...
        "all, so a follow on it is silently empty. **`mount` now defaults to "
        "false** -- this used to default to true, so a bare call that once "
        "mounted every unmounted thread it found is now a census and nothing "
        "else. That is deliberate: mounting raises the app's window over "
        "whatever the user is working in, once per thread, and asking what is "
        "reachable should not cost them that. Pass mount=true to actually "
        "focus them, and name the threads you mean when you do. Mounting is "
        "additive, not a swap -- bringing one forward does not evict the "
        "others -- so treat it as a one-off warm-up for the threads you intend "
        "to watch, NOT something to cycle through repeatedly. Returns each "
        "thread with `mounted` and its owning client, plus `mounted_by_sync` "
        "for the ones this call gained and `skipped` for the ones it could not "
        "mount, each with a `reason`: a thread one of our own detached runs is "
        "writing, one another writer holds, one listed off a writer lock with "
        "nothing on disk to bind it to yet, one whose instance has no nameable "
        "app to aim the link at, or `suppressed` when "
        "CODEX_PILOT_SUPPRESS_FOCUS is set. None of those fails the sweep -- "
        "the rest still mount."
    )
)
def sync_threads(
    threads: list[str] | None = None,
...
@server.tool(
    description=(
        ...
        "The deep link is aimed at the app actually serving that instance, since "
        "every installed Codex bundle claims the `codex://` scheme and macOS "
        "picks one handler -- an unaimed link can land in an app whose CODEX_HOME "
        "has never heard of the thread, and nothing reports it. `app` says which "
        "bundle it went to. When no app is serving the instance and none can be "
        "named for it, this refuses rather than firing a link that goes "
        "somewhere arbitrary.\n\n"
        "A thread the app is already showing gets no link at all: firing one "
        "would re-run the app's window restore to display what is already "
        "displayed. That comes back as focus.reason 'skipped_already_mounted' "
        "with `owner` set and `opened` null, and costs the user nothing. The "
        "check is a probe bounded to a second, because a mounted thread answers "
        "at once while an unmounted one takes the router's full ~10s to say so, "
        "so `owner` is null whenever nothing proved the thread mounted inside "
        "that second -- which is NOT the same as proving it unmounted. Do not "
        "read a null `owner` as a fact about the thread. activate=true skips "
        "the check, since wanting the window in front is a different request.\n\n"
        "When a link is fired it takes the screen and then hands it back: the "
        "app raises its own window when it handles the deep link, whatever "
        "activate is set to, so the frontmost app is noted first and reactivated "
        "once Codex is up. The `focus` field says whether that worked, confirmed "
        "by looking rather than assumed from the command -- restored:false with a "
        "reason when the user was already in Codex, moved on meanwhile, the probe "
        "failed, or the reactivation never took. 'not_raised' carries "
        "`waited_seconds`, which is 3 for an app already running and 15 when the "
        "link had to launch one, a launch reaching the screen long after a raise "
        "would.\n\n"
        "`surfaced` says whether the thread is now reachable over IPC, and is the "
        "field to branch on. It is false only when CODEX_PILOT_SUPPRESS_FOCUS is "
        "set, which turns deep links off entirely: nothing is fired, focus.reason "
        "is 'suppressed', and the thread stays exactly as undriveable as it was. "
        "Do not retry an IPC verb against it -- use read_thread, which works off "
        "the rollout and needs no window.\n\n"
        "Focusing is still an interruption, so focus to DRIVE a thread, never "
